...
function Teacheredit() {
  ...
  const admindetail =  JSON.parse(admintinfo)
    ...
    useEffect(() => {
        if (eid) {
            fetch(`${config.baseUrl}teacherss/${eid}`)
                .then((res) => res.json())
                .then((data) => {
                    ...
                    setmainStandard(data[0].mainstandard);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
            const url = `${config.baseUrl}teachers/${eid}`;

            fetch(url, {
                method: "PUT",
                headers: { "content-type": "application/json" },
                body: JSON.stringify({surname, fname, lname, zender,mainstandard, password,subject , age, birthdate, phonenumber: phoneNumber, standard, email })
            })
                .then((res) => res.json())
                .then((data) => {
                    console.log(data);
                    navigate('/teacher/' + admindetail._id);
                })
                .catch((error) => console.error('Error:', error));
    ...
    return (
        <div className="container mt-5 w-50 mx-auto border border-1 pt-2 shadow">
            <h1 className="text-center text-success mb-4">Edit Teacher</h1>
            <form onSubmit={handleSubmit}>
                ...
                <div className="mb-3">
                    <label htmlFor="age" className="form-label">Subject:</label>
                    <input type="text" className="form-control" id="age" value={subject} onChange={(e) => setsubject(e.target.value)} />
                </div>
                <div className="mb-3">
